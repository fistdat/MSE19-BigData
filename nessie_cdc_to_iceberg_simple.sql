-- NESSIE CDC TO ICEBERG (SIMPLE - NO UPSERT)
...
CREATE TABLE kafka_cdc_source (
    ...
    'properties.group.id' = 'flink-cdc-simple-consumer',
...
-- Create Nessie-backed Iceberg sink table (SIMPLE - NO UPSERT)
CREATE TABLE IF NOT EXISTS demographics_lakehouse_simple (
    operation STRING,
    record_id STRING,
    city STRING,
    state STRING,
...
WITH (
    'format-version' = '2'
    -- No UPSERT to avoid equality field error
);

SHOW TABLES;

-- Insert CDC data into Nessie-backed Iceberg table
INSERT INTO demographics_lakehouse_simple
SELECT 
    CASE 
