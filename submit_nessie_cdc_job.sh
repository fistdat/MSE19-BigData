#!/bin/bash
# =============================================================================
# SUBMIT NESSIE CDC TO ICEBERG JOB
# =============================================================================
# Complete CDC pipeline with Nessie catalog - working solution!
# =============================================================================

source pipeline_config.env

echo "🗄️ SUBMITTING NESSIE CDC TO ICEBERG JOB"
echo "======================================"
...
# Create job SQL with environment variable approach
cat > /tmp/nessie_cdc_complete.sql << 'EOF'
-- NESSIE CDC TO ICEBERG (FINAL WORKING VERSION)
...
docker cp /tmp/nessie_cdc_complete.sql ${CONTAINER_FLINK_JM}:/tmp/

echo "📤 Executing job with environment variable..."
JOB_RESULT=$(docker exec -e AWS_REGION=us-east-1 ${CONTAINER_FLINK_JM} /opt/flink/bin/sql-client.sh -f /tmp/nessie_cdc_complete.sql 2>&1)
JOB_EXIT_CODE=$?
...
if [ $JOB_EXIT_CODE -eq 0 ]; then
    ...
    if [ "$RUNNING_JOBS" -gt 0 ]; then
        ...
        INSERT_RESULT=$(docker exec ${CONTAINER_POSTGRES} psql -U ${PG_USER} -d ${PG_DATABASE} -c "INSERT INTO demographics (city, state, population_count) VALUES ('${TEST_CITY}', 'NessieState', 99999);" 2>&1)
